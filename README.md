...
`강의녹음.bat` 을 더블클릭하면 GUI 창이 뜹니다 (`pythonw` 로 실행되어 콘솔 창이 없습니다). 매번 명령어를 칠 필요가 없습니다.

1. **`강의녹음.bat` 더블클릭** → 잠시 후 "모델 로딩 완료" 표시
2. 영상을 **소리나게 재생** → **입력 레벨 미터가 초록(양호)** 인지 확인
3. **강의 이름** 입력 후 **● 녹음 시작**
4. 강의가 끝나면 **■ 중지** → **텍스트 파일 열기** 버튼으로 결과 확인

주요 기능:

- **실시간 입력 레벨 미터** — 빨강(소리 없음) / 노랑(낮음) / 초록(양호). 녹음 전에 볼륨이 충분한지 미리 확인할 수 있습니다.
- 60초마다 자동으로 청크 저장·변환, 결과는 `{강의이름}.txt` 에 누적 (CLI와 동일한 위치·형식)
- 무음 구간 자동 제외, 진행 로그 표시

> ⚠️ **입력 레벨 주의:** 시스템 오디오 loopback 캡처는 **시스템/영상 볼륨에 영향을 받습니다.** 볼륨이 너무 낮으면 레벨 미터가 빨강이 되고 인식 품질이 급격히 떨어집니다. 녹음 전 영상을 틀어 레벨이 **초록**인지 꼭 확인하세요. (헤드폰을 써도 캡처되며, 볼륨만 0이 아니면 됩니다.)

> tkinter 기반이라 별도 설치가 필요 없습니다 (Windows용 Python에 기본 포함).
...
```
wspr/
├── transcribe_lecture.py      # CLI 버전
├── lecture_gui.py             # GUI 버전
├── 강의녹음.bat               # GUI 더블클릭 실행 (pythonw, 콘솔 없음)
